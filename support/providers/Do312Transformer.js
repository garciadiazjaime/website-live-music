...
function Do312Pages(html, originalLink) {
  ...
  const nextPageLink = `${getOriginFromUrl(originalLink.url)}${$(".ds-next-page").attr("href")}`;
...
function Do312Transformer(html, link) {
  ...
  const events = $(".event-card")
    ...
    .map((item) => {
      ...
      const url = `${getOriginFromUrl(link.url)}${$(item).find(".ds-listing-event-title.url").attr("href")}`;
      const venue = $(item).find(".ds-venue-name").text().trim();
      const timestamp = $(item).find(".ds-listing-details > meta").attr("datetime");
